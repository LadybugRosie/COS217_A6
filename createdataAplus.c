...
/*
   ...
   name and the message "A+ is your grade." and thus assign an
   A+.

   The file overflows readString's stack buffer and overwrites
   getName's saved return address (x30) with the address of
   injected machine instructions built via miniassembler.h.
   Those injected instructions:
      - compute the address of a format string "A+ is your grade.%c",
      - load a character into w1 for the trailing %c,
...
int main() {
    ...
    */
    ...
    /*
       ...
       adrInstr: adr x0, <printString>
         - 0 is the destination register (x0).
         - 0x420060 is a base address in the name[] region.
         - 0x420074 is the target address where printString
           will reside.

       movInstr: mov w1, 0x0a
         - w1 receives the character that printf will print
           for the trailing "%c".

       bInstr: b <printf call in main>
         - Branches from our injected code at 0x42007c to the
           existing printf call at 0x4008ac.
    */
